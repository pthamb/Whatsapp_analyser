...
def emoji_helper(selected_user, df):
    # Filter messages by selected user if not 'Overall'
    if selected_user != 'Overall':
        df = df[df['user'] == selected_user]
    ...
    for message in df['message']:
        # Use the emoji library to find all emojis in the current message
        emojis = [char for char in message if emoji.is_emoji(char)]
        emoji_count.update(emojis)

    # Convert the emoji_count Counter to a DataFrame
